<script>
  ...
  $: switch ($page.url.pathname) {
    case "/":
      pageOffer = "Ремонт квартир в Москве";
      pageDescription =
        "Независимо от того, какой именно ремонт в квартире нужен, наши специалисты выполнят любые работы. Вы можете заказать ремонт одной из комнат, помещения или полностью квартиры под ключ – мы сделаем работу быстро и качественно.";
      break;
    case "/about":
      pageOffer = "Информация о нашей компании";
      pageDescription =
        "Для нас главным приоритетом является довольный клиент, поэтому мы выбираем и применяем лучшие технологические решения. В результате каждый, кто обращается к нам, на собственном примере убеждается – это действительно то, чего он хотел.";
      break;
    case "/price":
      pageOffer = "Цены на ремонт квартиры";
      pageDescription =
        "Стоимость наших услуг зависит от объема, а также сложности работ. Мы также разработали специальные программы лояльности, акции и другие бонусы, помогающие сэкономить. К тому же даже без скидок наш обычный прайс имеет доступные цены.";
      break;
    case "/objects":
      pageOffer = "Квартиры после нашего ремонта";
      pageDescription =
        "Здесь вашему вниманию представлены проекты выполненных работ по ремонту квартир разных стилей. Своей гордостью мы считаем максимальное внимание именно к деталям и мелочам, ведь именно из них состоит основа вашего будущего комфорта и удобства.";
      break;
    case "/blog":
      pageOffer = "Наш тематический блог о ремонте";
      pageDescription =
        "Мы собрали самые интересные статьи, из которых вы сможете узнать ценную информацию, касаемо квартирного ремонта и всем, что с ним связано. Независимо от того, что именно вас интересует, просто заходите сюда, и вы наверняка найдёте что-то интересное.";
      break;
    case "/contacts":
      pageOffer = "Контактные данные с сотрудниками";
      pageDescription =
        "Если по каким-либо причинам у вас остались или наоборот, появились вопросы, вы можете задать их нашим специалистам по телефону, почте или через чат. Мы практически все время на связи, поэтому ответ последует в максимально короткие сроки.";
      break;
    default:
      pageOffer = "Блог о ремонте";
      pageDescription =
        "Мы периодически добавляем посты на тему ремонта квартир. В них вы найдёте полезную информацию на ту или иную тему, а также полезные советы, которые вам помогут сделать ремонт более качественно и красиво.";
      break;
  }
</script>
  ...
  <div class="max-w-7xl mx-auto py-3 lg:px-8">
    <div class="text-center">
      <div class="font-medium text-white text-sm px-8">
        <span
          >Скидки постоянным клиентам - Работаем круглосуточно - Требуются
          мастера - Качество работ гарантируем</span
        >
      </div>
    </div>
  </div>
...
<header>
  <div class="relative pb-16 sm:pb-20">
    ...
    <div class="relative z-10 bg-slate-200 bg-opacity-40">
      <nav
        class="relative max-w-7xl mx-auto flex items-center justify-between p-2"
        aria-label="Global"
      >
        <div class="flex items-center justify-between w-full lg:w-auto">
          <a href="/">
            ...
            <img
              class="h-14 w-auto sm:h-14 transition ease-in-out delay-100 hover:scale-110 duration-500"
              src="/logo-header.png"
              alt="logo"
            />
          </a>
          <div class="-mr-2 flex items-center lg:hidden">
            <button
              ...
              type="button"
              class="bg-sky-800 bg-opacity-0 rounded-md p-2 inline-flex items-center justify-center text-sky-700 hover:text-sky-100 hover:bg-opacity-100 focus:outline-none focus:ring-2 focus-ring-inset focus:ring-white"
              aria-expanded="false"
            >
        ...
        <div class="hidden space-x-10 lg:flex lg:ml-10">
          {#each Menu as { value, link }}
            <a
              href="/{link}"
              on:click={() => gaspText(boxParam.box1)}
              class="text-base font-medium text-sky-900 hover:text-pink-900 transition ease-in-out delay-100 hover:scale-110 duration-500"
            >
              {value}
            </a>
          {/each}
        ...
        <div class="hidden lg:flex items-center px-2">
          <div class="">
            <div
              class="flex space-x-2 bg-sky-800 bg-opacity-10 px-3 py-2 rounded-xl"
            >
              <!-- Current: "bg-gray-900 text-white", Default: "text-gray-300 hover:bg-gray-700 hover:text-white" -->
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-7 w-7 text-sky-800 "
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                stroke-width="2"
              ...
              <a
                href="tel:+79060456444"
                class="text-sky-800 rounded-md text-xl font-bold "
                >+7 (980) 150-69-43</a
              >
            </div>
          </div>
    ...
    <div
      class="relative mt-24 mx-auto p-4 sm:max-w-4xl sm:mt-32 sm:px-6 lg:max-w-7xl items-center"
    >
      <!-- <div id="{boxParam.box1}" class="relative mt-24 mx-auto p-4 sm:max-w-4xl sm:mt-32 sm:px-6 lg:max-w-7xl items-center"> -->
      <h1
        class=" font-bold tracking-tight bg-clip-text bg-clip-text text-transparent bg-gradient-to-r from-cyan-600 via-yellow-500 to-yellow-600 text-5xl lg:text-6xl max-w-xl"
      >
        {pageOffer}
      </h1>
      <p
        class="mt-6 p-4 text-base sm:text-lg text-white max-w-xl bg-sky-800 bg-opacity-60 rounded-md"
      >
        {pageDescription}
      </p>
      <div class="">
        <button
          on:click={changeVisibleFormCalculation}
          class=" mt-8 pb-2 pt-1 px-6 bg-sky-800 bg-opacity-10 hover:bg-opacity-60 rounded-xl border border-transparent rounded-md text-2xl font-bold text-sky-900 hover:text-gray-100 "
        >
          Заказать расчёт
        </button>
      </div>
